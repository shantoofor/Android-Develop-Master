...
package com.shantoo.develop.library.ui.widget.materialloadingprogressbar;
...
public class CircleProgressBar extends android.support.v7.widget.AppCompatImageView {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        final TypedArray a = context.obtainStyledAttributes(
                attrs, R.styleable.MLCircleProgressBar, defStyleAttr, 0);

        final float density = getContext().getResources().getDisplayMetrics().density;

        mBackGroundColor = a.getColor(
                R.styleable.MLCircleProgressBar_mlpb_background_color, DEFAULT_CIRCLE_BG_LIGHT);

        mProgressColor = a.getColor(
                R.styleable.MLCircleProgressBar_mlpb_progress_color, DEFAULT_CIRCLE_BG_LIGHT);
        mColors = new int[]{mProgressColor};

        mInnerRadius = a.getDimensionPixelOffset(
                R.styleable.MLCircleProgressBar_mlpb_inner_radius, -1);

        mProgressStokeWidth = a.getDimensionPixelOffset(
                R.styleable.MLCircleProgressBar_mlpb_progress_stoke_width, (int) (STROKE_WIDTH_LARGE * density));
        mArrowWidth = a.getDimensionPixelOffset(
                R.styleable.MLCircleProgressBar_mlpb_arrow_width, -1);
        mArrowHeight = a.getDimensionPixelOffset(
                R.styleable.MLCircleProgressBar_mlpb_arrow_height, -1);
        mTextSize = a.getDimensionPixelOffset(
                R.styleable.MLCircleProgressBar_mlpb_progress_text_size, (int) (DEFAULT_TEXT_SIZE * density));
        mTextColor = a.getColor(
                R.styleable.MLCircleProgressBar_mlpb_progress_text_color, Color.BLACK);
        mShowArrow = a.getBoolean(R.styleable.MLCircleProgressBar_mlpb_show_arrow, false);
        mCircleBackgroundEnabled = a.getBoolean(R.styleable.MLCircleProgressBar_mlpb_enable_circle_background, true);

        mProgress = a.getInt(R.styleable.MLCircleProgressBar_mlpb_progress, 0);
        mMax = a.getInt(R.styleable.MLCircleProgressBar_mlpb_max, 100);
        int textVisible = a.getInt(R.styleable.MLCircleProgressBar_mlpb_progress_text_visibility, 1);
        if (textVisible != 1) {
            mIfDrawText = true;
        }
    ...
    public void setBackgroundColor(@ColorRes int colorRes) {
        if (getBackground() instanceof ShapeDrawable) {
            int color = ContextCompat.getColor(mContext, colorRes);
